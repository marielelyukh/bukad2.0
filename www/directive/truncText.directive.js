  ...
  'use strict';
    ...
    function truncText($filter) {
        return {
            ...
            template: '<p class="text" ng-click="changeLimit()">{{text | limitTo: localLimit}}</p><p style="color: #ccc; text-align: right;" ng-click="changeLimit()" ng-show="text.length > localLimit && localLimit == limit"> {{moreText}} </p>',
            link: function (scope) {
